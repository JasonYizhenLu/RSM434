...
s.headers.update({'X-API-key': 'V4PYCGCW'}) # convenience, we do not need to separately include the API Key in our messaging code as it is already included when we use "s"
...
def get_tick_status(): # this function queries the status of the case ("ACTIVE", "STOPPED", "PAUSED") which we will use in our "while" loop
    ...
    if resp.ok: 
        case = resp.json() 
        return case['tick'], case['status'] # the code does not use the "tick" value, but may be useful if the code is modified
...
def get_bid_ask(ticker): # this function queries the order book for price and volume data; specifically finds the best bid and offer (BBO) prices and the total volume bid and offered at the BBO (i.e. multiple orders may be placed at the same price, this function consolidates the volume over all orders at the BBO)
    ...
    if resp.ok:
        ...
        bid_volume_fn = sum([item['quantity'] - item['quantity_filled'] for item in bid_book if item["price"] == bid_price_fn])
        ask_volume_fn = sum([item['quantity'] - item['quantity_filled'] for item in ask_book if item["price"] == ask_price_fn]) 
...
def main():
    ...
    tick, status = get_tick_status() # this function call queries the case status and establishes whether the loop will start
    ...
    while status == 'ACTIVE': # the loop is the algo - the loop contains the set of instructions (including calls to the functions we define above) that execute our trading strategy; we want these instructions to be repeatedly executed over the duration of the case and use the "while" loop to imnplement this repetition - there are other types of loops that can be used, such as a "for" loop
        ...
        shares_to_trade = min(shares_to_trade, abs(TARGET_SHARES - current_position)) # an error check to make sure we do not enter an order volume that would cause our position to exceed +/- 100,000 shares
        ...
        if TARGET_SHARES > 0 and current_position != TARGET_SHARES: # "if statements are common triggers for launching orders - the order will be sent if the conditions are true; in this case we are identifying that we are buying (TARGET_SHARES > 0) and that we have not yet filled the order (current_position != TARGET_SHARES)
            ...
            resp = s.post('http://localhost:9999/v1/orders', params = {'ticker': TICKER_SYMBOL, 'type': 'LIMIT', 'quantity': shares_to_trade_active, 'price': ask_price, 'action': 'BUY'}) # active limit order - the order is intended to be marketable as the price is the BBO ask price
            resp = s.post('http://localhost:9999/v1/orders', params = {'ticker': TICKER_SYMBOL, 'type': 'LIMIT', 'quantity': shares_to_trade_passive, 'price': bid_price, 'action': 'BUY'}) # passive limit order - the order is not intended to be marketable as the price is the BBO bid price
        ...
        tick, status = get_tick_status() # update the status of the case for our "while" loop - if we do not update the status the loop will continue forever once it starts (unless an error breaks the execution)
